...
class ParallelTube extends TriangleMesh {
  ...
  List<Vec3D> getCircle(int i, float _radius) {
    ...
    for (int j=0; j<=k; j++) {
      ...
      Vec3D p = vert.get(j);
      p.x = soul.getVertices().get(i).x + c*soul.getBinormal(i).x + s*soul.getNormal(i).x;
      p.y = soul.getVertices().get(i).y + c*soul.getBinormal(i).y + s*soul.getNormal(i).y;
      p.z = soul.getVertices().get(i).z + c*soul.getBinormal(i).z + s*soul.getNormal(i).z;
